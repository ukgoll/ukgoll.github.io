...
栈区 stack，堆区 heap
由图我们可以看出来，stack 的地址是从高地址的向低地址增长的（这就是涉及到一个问题了，stack 里变量的地址是开始地址还是结束地址），heap 的地址由低到高增长的，用于动态分配内存的区域，stack 和 heap 之间可用空间还是很大的。 （**对于系统一些没有硬件支持的 stack，C 语言可能会使用链表使用 stack 栈区**）
